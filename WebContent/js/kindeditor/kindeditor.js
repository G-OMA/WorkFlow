...
(function(window, undefined) {
	...
	var _VERSION = '4.1.5 (2013-01-20)', _ua = navigator.userAgent
	...
	function _toMap(val, delimiter) {
		...
		_each(arr, function(key, val) {
			if ((match = /^(\d+)\.\.(\d+)$/.exec(val))) {
				for ( var i = parseInt(match[1], 10); i <= parseInt(match[2],
						10); i++) {
					map[i.toString()] = true;
				}
			} else {
	...
	KEditor.prototype = {
		...
		create : function() {
			...
			if (fullscreenMode) {
				K(window).bind(
						...
						function(e) {
							if (self.isCreated) {
								newResize(_docElement().clientWidth,
		...
		resize : function(width, height) {
			...
			if (height !== null && self.toolbar.div && self.statusbar) {
				height = _removeUnit(height) - self.toolbar.div.height()
